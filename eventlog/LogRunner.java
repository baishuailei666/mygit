package com.xlauncher.utils.eventlog;
    ...
    public void run() {
        try {
            while (true) {
                ...
                // timeSpan 输出应用程序启动到输出日志信息消耗的毫秒数
                ...
                if (timeSpan >8000) {
                    ...
                    alertLog.setPriority(String.valueOf(event.getLevel()));
                    alertLog.setTime(logDateTime);
                    alertLog.setTimeSpan((int) timeSpan);
                    alertLog.setThread(String.valueOf(event.getThreadName()));
                    alertLog.setLineNumber(event.getLocationInformation().getLineNumber());
                    alertLog.setFileName(event.getLocationInformation().getFileName());
                    alertLog.setClassName(event.getLocationInformation().getClassName());
                    alertLog.setMethodName(event.getLocationInformation().getMethodName());
                    alertLog.setMessage((String) event.getMessage());
...
//                eventMap.put("输出应用程序启动到输出日志信息消耗的毫秒数",timeSpan);
//
...
////                String url = "http://localhost:8080/AlertLog/add";
////                rest.postForEntity(url,eventMap, eventMap.getClass());
//
