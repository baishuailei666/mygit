...
//        jdbcutil.getCount("2019-03-20 14:00:00", "2019-03-25 14:00:00");
     ...
     * @return
     */
    private byte[] findPredictImage(String predictTime) {
        ...
        try {
            if (predictTime == null) {
                System.out.println("predictTime is null");
            }
            SimpleDateFormat simpleDateFormat = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
            Date date = simpleDateFormat.parse(predictTime);
            findFileName = FACE_PHOTO + String.valueOf(date.getTime());
            System.out.println("MongoDB查询图片! findFileName:" + findFileName);
        } catch (ParseException e) {
            System.err.println("数据格式错误!" + e);
        }
        ...
        try {
            ...
            System.err.println("Err.数据流错误!" + e);
        } finally {
            mongoClient.close();
        }
        return bytes;
    ...
    private void getImage(String startTime, String endTime, String path, int code) throws Exception {
        ...
        if (count != 0) {
            ...
            if (code == 1) {
                ...
                // syn_user
                System.out.println("code is 2, 执行存储抓拍人脸图片程序!");
                sql = "SELECT predict_time FROM face_predict WHERE face_photo IS NOT NULL AND predict_time > ' " + startTime + "' AND predict_time < ' " + endTime + "'";
                System.out.println("FACE SQL。" + sql);
                try {
                    // 获取Statement
                    ...
                    while (rs.next()) {
                        index ++;
                        String predictTime = rs.getString(1);
                        byte[] bytes = findPredictImage(predictTime);
    ...
    private int getCount(String startTime, String endTime, int code) throws Exception {
        ...
        if (code == 1) {
            ...
            // predict_time
            count = "SELECT COUNT(predict_time) FROM face_predict WHERE predict_time > ' " + startTime + "' AND predict_time < ' " + endTime + "'";
        }
        try {
            // 获取Statement
            st = con.createStatement();
