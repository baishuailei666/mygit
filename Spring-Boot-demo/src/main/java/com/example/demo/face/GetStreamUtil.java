...
public class GetStreamUtil {
    ...
    private static void getPicture(String ip, short port, String userName, String password) throws InterruptedException {
        NativeLong nativeLong = new NativeLong(1);
        ...
        HCNetSDK.FMSGCallBack_V31 fmsfcallbackV31 = (lCommand, pAlarmer, pAlarmInfo, dwBufLen, pUser) -> {
            ...
            switch (lCommand.intValue()) {
                case HCNetSDK.COMM_UPLOAD_FACESNAP_RESULT:
                    ...
                    float fx = netVcaFacesnapResult.struRect.fX;
                    float fy = netVcaFacesnapResult.struRect.fY;
                    float fwidth = netVcaFacesnapResult.struRect.fWidth;
                    float fheight = netVcaFacesnapResult.struRect.fHeight;
                    int iId = netVcaFacesnapResult.dwFacePicID;
                    float fScore = netVcaFacesnapResult.dwFaceScore;

                    System.out.println("f_x." + fx + " f_y." + fy + " f_width." + fwidth + " f_height." + fheight+
                            " id:"+iId + " score:" + fScore);
                    ...
                    if (netVcaFacesnapResult.dwBackgroundPicLen >0 && netVcaFacesnapResult.pBuffer2 != Pointer.NULL) {
                        ...
                        try {
                            System.out.println("保存bytes图片");
                            // byte数组保存图片

                            String fileName = "D:\\sdkimage\\face-" + DateTimeUtil.getFormatTime(System.currentTimeMillis()) +".jpg";
                            FileImageOutputStream imageOutput = new FileImageOutputStream(new File(fileName));
                            imageOutput.write(bytes, 0, bytes.length);
                            imageOutput.close();

                            MatUtil.matUtil(fileName, fx, fy, fwidth, fheight);
                        } catch (IOException e) {
                            e.printStackTrace();
                        }
